lang=cpp
private_test_case_path=../processed_data/public_test_cases
public_test_case_path=../processed_data/generated_test_cases
...
generation_path=../output/output
api_idx=0
restart_pos=0

mkdir -p ../output/$lang/$base_mode
cp ../output/$lang/initial_results_$model_name.jsonl ../output/$base_mode/results.jsonl
...
do
  ...
  if [ $? -eq 0 ]; then
    ...
    output_path=../output
    mode=$base_mode
    python mergec.py --iteration $i --generation_number $generation_number  --lang $lang --model_name $model_name --mode $mode --generation_path $generation_path --beam_number $beam_number
  else
