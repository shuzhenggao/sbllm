...
def evaluate_generated_outputs(cfg: EvaluationConfig) -> None:
    ...
    problem_id_to_ground_truths, output_code_location = write_programs_read_ground_truth(cfg, merged)
    ...
    if cfg.run_reference:
        tag_to_path.append(("reference", f"_reference.{lang_file_ending}"))
    if cfg.run_input:
        tag_to_path.append(("input", f"_slow.{lang_file_ending}"))
    ...
    running_tags = ' | '.join([tag for (tag, _) in tag_to_path])
    logging.info(f"running: {running_tags}")
    
    #results = run_programs(cfg, merged, problem_id_to_ground_truths, output_code_location, tag_to_path)
    results = run_programs_multi_progress(cfg, merged, problem_id_to_ground_truths, output_code_location, tag_to_path)
    ...
    if not cfg.run_reference or not cfg.run_input:
        merged = read_input_and_reference_info(cfg, merged)
...
def read_input_and_reference_info(cfg, merged):
    preprocessed_df = pd.read_json(cfg.preprocessed_output_file, lines=True, orient="records")
    merged = pd.merge(
        merged,
        preprocessed_df,
...
def read_inputs_and_prepare(cfg) -> pd.DataFrame:
    ...
    logging.info(f"Running each program {cfg.num_trials} times, skipping the first {cfg.ignore_first_k} runs, and getting the input-output pairs from {cfg.inputs_outputs_basepath}")
    
    gen_df = pd.read_json(cfg.model_generated_outputs_path, lines=True, orient="records")
    ...
    if cfg.is_prompt_based:
        gen_df["slower_program"] = gen_df.apply(lambda x: get_input_from_prompt(x), axis=1)
    else:
    ...
    if cfg.reference_file_path is not None:
        ...
        # entire input
        ref_df["slower_program"] = ref_df[cfg.reference_input_col].apply(lambda x: x.strip().replace("\n\n\n\n\n", ""))
        gen_df["slower_program"] = gen_df["slower_program"].apply(lambda x: x.strip().replace("\n\n\n\n\n", ""))
        
        logging.info(f"Inputs in reference: {len(ref_df['slower_program'])}")
        logging.info(f"Inputs in generation: {len(gen_df['slower_program'])}")
        logging.info(f"Unique inputs in reference: {len(ref_df['slower_program'].unique())}")
        logging.info(f"Unique inputs in generation: {len(gen_df['slower_program'].unique())}")
        assert len(ref_df["submission_id_v0"].unique()) == len(ref_df), "submission_id_v0 should be unique"

        ref_df = ref_df.drop_duplicates(subset=["slower_program"])
        gen_df = gen_df.drop_duplicates(subset=["slower_program"])
        merged = pd.merge(
            gen_df,
            ref_df,
            left_on="slower_program",
        ...
        assert (cfg.reference_code_col in gen_df.columns), f"Column {cfg.reference_code_col} not found in {cfg.model_generated_outputs_path}"
        merged = gen_df
        
        
        merged = merged[merged[cfg.slow_code_col] != merged[cfg.reference_code_col]]

    assert (len(merged) > 0), f"{cfg.slow_code_col} and {cfg.reference_code_col} are the same for all programs"
...
def write_programs_read_ground_truth(
    ...
    pattern = r"eval\s*\(\s*input\s*\(\s*\)\s*\)"
    ...
    for number in range(cfg.cpu_number, cfg.cpu_number+cfg.process_number+1):
        if not os.path.exists(os.path.join(output_code_location, str(number))):
            os.makedirs(os.path.join(output_code_location, str(number)))
    ...
    for _, row in tqdm(merged.iterrows(), total=len(merged), desc="writing programs"):
        ...
        if problem_id not in problem_id_to_ground_truths:
            num_test_cases = len(glob.glob(f"{cfg.inputs_outputs_basepath}/{problem_id}/output*.txt"))
            if num_test_cases==0:
                print(problem_id)
                continue
            assert (num_test_cases > 0), f"{cfg.inputs_outputs_basepath}/{problem_id} has no ground truth files!"
        ...
        for number in range(cfg.cpu_number, cfg.cpu_number+cfg.process_number+1):
            with open(os.path.join(output_code_location, str(number), f"{submission_id_v0}_{problem_id}_slow.{lang_file_ending}"), "w") as f:
                ## This change in order to keep the comments out
                if True or cfg.language=='cpp' or row["slower_program"].strip()=='print((input()))':
                    f.write(row["slower_program"].strip())
                else:
                    f.write(re.sub(pattern, "input()", row["slower_program"].strip()))
        ...
        for i, generated_program in enumerate(generated_programs):
            for number in range(cfg.cpu_number, cfg.cpu_number+cfg.process_number+1):
                with open(os.path.join(output_code_location, str(number), f"{submission_id_v0}_{problem_id}_maybe_faster_{i}.{lang_file_ending}"),"w") as f:
                    if True or cfg.language=='cpp' or generated_program.strip()=='print((input()))':
                        f.write(generated_program.strip())
                    else:
                        f.write(re.sub(pattern, "input()", generated_program.strip()))
                    #f.write(generated_program.strip())

        for number in range(cfg.cpu_number, cfg.cpu_number+cfg.process_number+1):
            with open(os.path.join(output_code_location, str(number), f"{submission_id_v0}_{problem_id}_reference.{lang_file_ending}"), "w") as f:
                if True or cfg.language=='cpp' or row[cfg.reference_code_col].strip()=='print((input()))':
                    f.write(row[cfg.reference_code_col].strip())
                else:
                    f.write(re.sub(pattern, "input()", row[cfg.reference_code_col].strip()))
                #f.write(row[cfg.reference_code_col].strip())
...
def run_programs_multi_progress(
    ...
    processes = int(cfg.process_number)
    pool = multiprocessing.Pool(processes)

    chunks_merged = np.array_split(merged, processes)
    chunks_data = []
    for i in range(processes):
        tmp_data={}
        tmp_data['cfg'] = deepcopy(cfg)
        tmp_data['cfg'].cpu_number+=i
        tmp_data['merged'] = chunks_merged[i]
        tmp_data['problem_id_to_ground_truths'] = problem_id_to_ground_truths
        tmp_data['output_code_location'] = output_code_location
        tmp_data['tag_to_path'] = tag_to_path
        chunks_data.append(tmp_data)

    results = pool.map(run_programs_single, chunks_data)
...
def run_programs_single(chunks_data: Dict):
    cfg = chunks_data['cfg']
    merged = chunks_data['merged']
    problem_id_to_ground_truths = chunks_data['problem_id_to_ground_truths']
    output_code_location = chunks_data['output_code_location']
    tag_to_path = chunks_data['tag_to_path']
        ...
        try:
            ...
            for (tag, suffix) in tag_to_path:
                code_path = os.path.join(
                    output_code_location, str(cfg.cpu_number), f"{submission_id_v0}_{problem_id}{suffix}"
                )
            ...
            results[item_id] = problem_execution_stats
...
def run_programs(
    ...
    # If we change this to also include (A, C), then we need to change the logic here. The following
    # assert checks that this is the case.
    assert len(merged["slower_program"].unique()) == len(
        merged
    ), f"Every row should have a unique submission_id_v0. This is not the case: number of unique submission_id_v0: {len(merged['submission_id_v0'].unique())}, number of rows: {len(merged)}"
...
def print_summary(cfg, merged, results, gen_col: str):
    ...
    for _, row in tqdm(merged.iterrows(), total=len(merged)):
        ...
        report_row.update(results[item_id])
        report_rows.append(report_row)
    logging.info(f" len(results) {str(len(results))}  len(report_rows) {str(len(report_rows))}")
    ...
    run_metrics = run_metrics[(run_metrics[f"{gen_col}_acc"] > 0.99) & (run_metrics["input_acc"] > 0.99)]
    if run_metrics.empty:
        return

    logging.info("---Execution time---")
    logging.info(f"Number of cases where the generated program is correct: {len(run_metrics)}")
    logging.info(f"[Reported in CodeNet] input program (ms): {mean_std(run_metrics, 'cpu_time_v0')}")
    logging.info(f"[Reported in CodeNet] reference (output) program (ms): {mean_std(run_metrics, 'cpu_time_v1')}")

    logging.info("-" * 80)
    logging.info(f"[Our measurement] input program (ms): {mean_std(run_metrics, 'input_time')}")
    logging.info(f"[Our measurement] reference (output) program (ms): {mean_std(run_metrics, 'reference_time')}")
    logging.info(f"[Our measurement] {gen_col} program (ms): {mean_std(run_metrics, f'{gen_col}_time')}")
    logging.info(f"[Our measurement] The speedup rate: {run_metrics['input_time_mean'].mean()/run_metrics[f'{gen_col}_time_mean'].mean()}")
    logging.info(f"[Our measurement] The runtime reduction: {1-run_metrics[f'{gen_col}_time_mean'].mean()/run_metrics['input_time_mean'].mean()}")
    ...
    if len(run_metrics_improved) > 0:
        ...
        logging.info(f"Found {len(run_metrics_improved)} problems where the {gen_col} program is faster than the input program")
        logging.info(f"[Our measurement] input program (ms): {mean_std(run_metrics_improved, 'input_time')}")
        logging.info(f"[Our measurement] reference (output) program (ms): {mean_std(run_metrics_improved, 'reference_time')}")
        logging.info(f"[Our measurement] {gen_col} program (ms): {mean_std(run_metrics_improved, f'{gen_col}_time')}")
    logging.info(f"Number of cases where reference took longer by our measurement: {len(get_anomalies(run_metrics))}")
