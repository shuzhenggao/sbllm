...
def testing_and_reporting(cfg):
    print('mapping......')
    input_code_map = {}
    with jsonlines.open("../processed_data/pie/{}_splits/test.jsonl".format(cfg.lang)) as f:
        for obj in f:
            input_code_map[remove_comments_and_docstrings(obj['code_v0_no_empty_lines'], cfg.lang)] = obj['input']
    #本身test data去重后就只有986个
    ...
    data['reference_file_path'] = "../processed_data/pie/{}_splits/test.jsonl".format(cfg.lang)
    data['output_report_file_path'] = os.path.join(cfg.output_path, cfg.mode, 'test_execution_{}.report'.format(cfg.model_name+'test'))
    data['preprocessed_output_file'] = "../processed_data/pie/{}_splits/processed_time.reports".format(cfg.lang)
    data['num_problems_to_evaluate'] = -1
    data['num_trials'] = 8
    data['ignore_first_k'] = 1
    data['max_time_per_run'] = 10
    data['temp_dir'] = "../processed_data/pie/{}_splits/generated_tmp".format(cfg.lang)
    ...
    if not os.path.exists(os.path.join(cfg.output_path, cfg.mode, 'test_execution_{}.report'.format(cfg.model_name+'test'))):
        cmd = 'cd ../pie/pie-perf-main; python src/codenet_eval/run_eval_feedback.py --eval_config {}'.format(os.path.join(cfg.output_path, cfg.mode, 'eval_config.yaml'))
