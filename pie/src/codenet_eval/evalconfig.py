...
class EvaluationConfig:
    ...
    # optional parameters    
    return_if_acc_below: float = 0.0 #  type: ignore | if the accuracy is below this value, then return immediately
    cpp_results_path: str = None #  type: ignore | the path to the results of compiling and running the ref cpp code
    cflags: str = "--std=c++17 -O1" #  type: ignore | the cflags to use for compiling the code
    run_reference: bool = True 
    run_input: bool = True 
    preprocessed_output_file: str = '' 
    reference_input_col: str = ''
    ...
    def from_args(cls, args):
        return cls(
            ...
            preprocessed_output_file=args.preprocessed_output_file,
            reference_input_col=args.reference_input_col,
    ...
    def get_args() -> argparse.ArgumentParser:
        ...
        args.add_argument("--run_reference", type=bool, default=True)

        args.add_argument("--run_input", type=bool, default=True)

        args.add_argument("--preprocessed_output_file", type=str, help="the preprocessed file with the running time of input and reference")

        args.add_argument("--reference_input_col", type=str)

        args.add_argument("--process_number", type=int, help="for multi process")
